...
public class JWTFilter extends GenericFilterBean {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        // TODO: Verificar si el token no es nulo/vacio y si empieza con el prefijo "Bearer "
        if (StringUtils.hasText(bearerToken) && bearerToken.startsWith("Bearer ")) {
            // Eliminar el prefijo "Bearer " para obtener solo el token
            String token = bearerToken.substring(7);

            // TODO: Utilizar el TokenProvider para obtener la autenticacion a partir del token JWT
            Authentication authentication = tokenProvider.getAuthentication(token);

            // TODO: Establecer la autenticacion en el contexto de seguridad de Spring para la solicitud actual
